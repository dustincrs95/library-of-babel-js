...
class LibraryOfBabel {
  ...
  seededRNGGenerator = (seed) => {
    ...
    const newRNGFunction = (min = 0, max = 1) => {
  ...
  search = (searchString: string, searchType: 'exact' | 'partial') => {
    ...
    const wall = '' + parseInt('' + (Math.random() * 3 + 1)); // 1 <= shelf <= 5

    const shelf = '' + parseInt('' + (Math.random() * 4 + 1)); // 1 <= volume <= 32

    const volume = ('' + parseInt('' + (Math.random() * 31 + 1))).padStart(2, '0'); // 1 <= page <= 410

    const page = ('' + parseInt('' + (Math.random() * 409 + 1))).padStart(3, '0'); // Add the above together and calculate the hash

    const locHash = this.calculateHash(wall + shelf + volume + page);
    let hex = ''; // The "depth" (character index on the page) at which the search result is found.
    ...
    const depth = parseInt('' + Math.random() * (this.pageLength - searchString.length)); // For every character in the page up until the index where we "find" the search result, put in random characters.
    ...
    for (let i = 0; i < searchString.length; i++) {
      const index = this.digs.indexOf(searchString[i]); // for each calculated value of the rng, it will be added to the index value and modded to len of an

      const rand = this.seededRNG(0, this.digs.length);
      const newIndex = this.modulo(index + parseInt('' + rand), this.an.length);
      const newChar = this.an[newIndex]; //hex will be built from the indexes translated into an
  ...
  getPage = (address: string) => {
    ...
    const page = addressArray[4]; // Calculate the hash of the location8

    const locHash = this.calculateHash(wall + shelf + volume.padStart(2, '0') + page.padStart(3, '0')); // The hash is used to seed the RNG

    this.seed = Math.abs(locHash);
    let result = '';

    for (let i = 0; i < hex.length; i++) {
      const index = this.an.indexOf(hex[i]); // for each calculated value of the rng, it will be subtracted from the index value and modded to len of digs

      const rand = this.seededRNG(0, this.an.length);
      const newIndex = this.modulo(index - parseInt('' + rand), this.digs.length);
      const newChar = this.digs[newIndex]; // document will be built from the indexes translated into digs
  ...
  getTitle = (address: string) => {
    ...
    const wall = addressArray[1];
    const shelf = addressArray[2];
    const volume = addressArray[3]; // We do +4 here instead of using the title from the address
